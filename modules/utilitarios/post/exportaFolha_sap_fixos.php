<?php
    ...
    $fileName = "exportaSapFixos.txt"; //$_FILES["file"]["tmp_name"][0];
   ...
   // var_dump(listaFolha());
    $enc = 0;
    foreach (listaFolhaFixos($dtIni, $dtFin) as $motorista){
        $posArray = array();
//echo "88888888".$motorista[0];
        ...
        $arLines[] = generatePosLine($matricula, DD::EV_QUESTOR_DIARIA, $hoUtils->numberFormat($motorista['DIARIAS']));
        $arLines[] = generatePosLine($matricula, DD::EV_QUESTOR_COMISSAO, number_format($motorista['COMISSAO'], 2, '', ""));
        ...
        $arLines[] = generatePosLine($matricula, DD::EV_QUESTOR_CREDITO, number_format($motorista['CREDITOS'], 2, '', ""));//$motorista['CREDITOS']); //FFB
        $arLines[] = generatePosLine($matricula, DD::EV_QUESTOR_ADIANTAMENTO, number_format($motorista['DEBITOS'], 2, '', ""));//$$motorista['DEBITOS']); //FFB  EV_QUESTOR_ADIANTAMENTO EV_QUESTOR_CREDITO
       // $arLines[] = generatePosLine($matricula, DD::EV_QUESTOR_DIFCREDEB, $motorista['DIFCREDEB']); //FFB DIFERENÇA ENTRE CRÉDITOS E DÉBITOS
//echo ">>>>>".$motorista['HORAESPERA'];        
    }
    ...
    if ($fileName){
        ...
        try {
            ...
            $hFile = fopen("C:/inetpub/wwwroot/bid/producao/modules/utilitarios/post/exportaSapFixos.txt", "w");            
            foreach ($arLines as $posLine){
                if ($posLine != null){
                    $line = "";
                    
                if($posLine['evento'] == "102"){
                    $posLine['evento'] = "35";                    
                }                    
                if($posLine['evento'] == "103"){
                    $posLine['evento'] = "49";                    
                } /*  alterado conforme solicitação por email em 16/04/2018 
                if($posLine['evento'] == "104"){
                    $posLine['evento'] = "96";                    
                }   */
                if($posLine['evento'] == "96"){
                    $posLine['evento'] = "104";                    
                }
                    ...
                    if(($posLine['evento'] == "106") or ($posLine['evento'] == "191") or ($posLine['evento'] == "108") or ($posLine['evento'] == "071")){
                            if(($posLine['evento'] == "071")){
                                    ...
                                    $line .= str_pad($posLine['valor'], 5, "0", STR_PAD_LEFT) . ";"; // Valor da referência
                                    $line .= str_pad($posLine['referencia'],      9, "0", STR_PAD_LEFT) . ";"; // Valor do evento
                                    $valor = str_pad($posLine['valor'], 9, "0", STR_PAD_LEFT);
                                    $referencia = str_pad($posLine['referencia'], 5, "0", STR_PAD_LEFT);
                                    if(($valor == "0000000000") and ($referencia == "00000")){
                                        $line = "";
                                    }else{
                                        $line .= PHP_EOL;
                                    }
                                ...
                                $evento = str_pad($posLine['evento'],     3, "0", STR_PAD_LEFT); 
                                if($evento == "191"){
                                    $evento = "099";
                                }
                                    $line .= "002;"; // Código da empresa no Questor
                                    $line .= str_pad($posLine['matricula'],  6, "0", STR_PAD_LEFT) . ";"; // Código do funcionário
                                    $line .= $evento . ";"; // Código do evento na Questor
                                    $line .= str_pad($posLine['valor'], 5, "0", STR_PAD_LEFT) . ";"; // Valor da referência
                                    $line .= str_pad($posLine['referencia'],      9, "0", STR_PAD_LEFT) . ";"; // Valor do evento
                                    $valor = str_pad($posLine['valor'], 9, "0", STR_PAD_LEFT);
                                    $referencia = str_pad($posLine['referencia'], 5, "0", STR_PAD_LEFT);
                    ...
                    }elseif($posLine['evento'] == "101"){
                        $valor = str_pad($posLine['referencia'],      9, "0", STR_PAD_LEFT);
                        if($valor <> "000000000"){ //pra evitar os eventos 101 zerados                              
                            $line .= "002;"; // Código da empresa no Questor
                            $line .= str_pad($posLine['matricula'],  6, "0", STR_PAD_LEFT) . ";"; // Código do funcionário
                            $line .= str_pad($posLine['evento'],     3, "0", STR_PAD_LEFT) . ";"; // Código do evento na Questor
                            $line .= str_pad($posLine['valor'], 5, "0", STR_PAD_LEFT) . ";"; // Valor da referência
                    ...
                    }elseif(($posLine['evento'] == "104") or ($posLine['evento'] == "105")){
                        $line .= "002;"; // Código da empresa no Questor
                        $line .= str_pad($posLine['matricula'],  6, "0", STR_PAD_LEFT) . ";"; // Código do funcionário
                        $line .= str_pad($posLine['evento'],     3, "0", STR_PAD_LEFT) . ";"; // Código do evento na Questor
                        $line .= str_pad($posLine['referencia'], 5, "0", STR_PAD_LEFT) . ";"; // Valor da referência
                        $line .= str_pad($posLine['valor'], 9, "0", STR_PAD_LEFT) . ";"; // Valor da referência
                        $valor = str_pad($posLine['valor'], 9, "0", STR_PAD_LEFT);
                        $referencia = str_pad($posLine['referencia'], 5, "0", STR_PAD_LEFT);
                        if(($valor == "0000000000") and ($referencia == "00000")){
                            $line = "";
                        ...
                        $linha = str_pad($posLine['evento'],     3, "0", STR_PAD_LEFT) . ";"; // Código do evento na Questor
                        $linha .= str_pad($posLine['referencia'], 5, "0", STR_PAD_LEFT) . ";"; // Valor da referência
                        $linha .= str_pad($posLine['valor'],      9, "0", STR_PAD_LEFT) . ";"; // Valor do evento
                            
                        if($linha <> "100;000000;000000000;"){ //pra evitar os eventos 100 zerados           
                            $line .= "002;"; // Código da empresa no Questor
                            $line .= str_pad($posLine['matricula'],  6, "0", STR_PAD_LEFT) . ";"; // Código do funcionário
                            $line .= str_pad($posLine['evento'],     3, "0", STR_PAD_LEFT) . ";"; // Código do evento na Questor
                            $line .= str_pad($posLine['referencia'], 5, "0", STR_PAD_LEFT) . ";"; // Valor da referência

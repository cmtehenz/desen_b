...
<html class="no-js" lang="pt-br">
    <head>
        <title>BID - Pagamentos semanal</title>
    ...
    <body>
        <?php
            ...
            for ($i = 0; $i < 7; $i++){
                ...
                $totalDia[$i] = contasPagarPagamentosPeriodo($diasSemana[$i], NULL, 'pago');
            }

            $totalSemana = numberFormatDB2(array_sum($totalDia));
            ...
            $sqlFornecedores = "SELECT
                    CAST(F.CNPJ_CPF AS VARCHAR(8)) AS CNPJ, SUM(P.VALOR_PARCELA + COALESCE(P.JUROS, 0) - COALESCE(P.DESCONTO, 0))
                FROM PARCENT P
                JOIN NOTAENT N ON N.ID_NOTAENT = P.ID_NOTAENT
                JOIN FORNECED F ON N.ID_FORNECED = F.ID_FORNECED
                WHERE
                    P.DT_PAGAMENTO BETWEEN '$diasSemana[0]' AND '$diasSemana[6]'
                    AND P.STATUS = 'P'
            ...
            foreach ($listaFornecedores as $key => $fornecedor){
                ...
                for ($i = 0; $i < 7; $i++) $dia[$i] = contasPagarPagamentosPeriodo($diasSemana[$i], NULL, 'pago', $fornecedor[0]);

                // Monta o link para a listagem de notas
                $auxHref = "sublists/nota.php?forneced=$idForneced&opcao=pago"
                    . "&dtIni=" . dateFormat($diasSemana[0])
                    . "&dtFin=" . dateFormat($diasSemana[6]);

                $linhaTabela .= "<tr class='gradeA'>";
                $linhaTabela .= "<td><a href='$auxHref'>$razaoSocial</a></td>";
        ...
        <div id="wrapper">
            ...
            <div id="content">
                ...
                <div class="container">
                    <div class="grid-24">
                        <form action="#" method="post" name="formPagamentosSemanal">
                            <div class="field">
                                <label>Selecione a semana:</label>
                                <select name="selecao_semana" id="selecao_semana" onchange="document.formPagamentosSemanal.submit()">
                        ...
                        <div class="widget widget-table">
                            <div class="widget-header">
                                ...
                                <h3 class="icon chart">PAGAMENTOS SEMANAL</h3>
                            </div>

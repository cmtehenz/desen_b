...
<html class="no-js" lang="pt-br">
    <head>
        <title>BID - Contas a pagar - Pago</title>
    ...
    <body>
        <?php
            ...
            $sqlMesSelecionado = mssql_query("SELECT * FROM mes WHERE id_mes = $mes");
            $dadosMesSelecionado = mssql_fetch_array($sqlMesSelecionado);

            // Total cumulativo
            $dadosCumulativo = contasPagarPagoAnoMes($ano, $mes);

            // Totais por fornecedor
            $coluna = "P.VALOR_PARCELA + COALESCE(P.JUROS, 0) - COALESCE(P.DESCONTO, 0)";
            $filtro = " AND P.STATUS = 'P' ";

            $dadosFornecedor = contasPagarSumFornecedorAnoMes($coluna, $ano, $mes, $filtro);

            foreach ($dadosFornecedor as $key => $value){
                $value[2] = numberFormatDB2($value[2]);
                $value[1] = utf8_encode($value[1]);

                $linhaTabela .=
                    "<tr class='gradeA'>
                        <td><a href='nota.php?mes=$mes&ano=$ano&forneced=$value[0]&opcao=pago'>$value[1]</a></td>
                    ...
                    <div class="grid-24">
                        <div class="widget widget-table">
                            <div class="widget-header">
                                ...
                                <h3 class="icon chart">CONTAS A PAGAR - PAGO</h3>
                            </div>
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    <tr class="odd gradeX">
                                        ...
                                        <td align='right'><?php echo $dadosCumulativo; ?></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div> <!-- .box -->

...
<html class="no-js" lang="pt-br">
    <head>
        <title>BID - Receita terceiros</title>
...
//RELACAO DE PLACA
            $sql_placa = "SELECT ID, PL,MODE, CONJU, SUM(FPESO), SUM(FRETEP) FROM
(SELECT HVEICULO.ID_VEICULO AS ID, HVEICULO.PLACA AS PL, MODELO.NAME AS MODE, CONJUNTO.NAME AS CONJU, SUM(VALTOTFRETE) AS FPESO, SUM(CT.VALFPESO) AS FRETEP
FROM CT
    JOIN HVEICULO ON (HVEICULO.ID_HVEICULO = CT.ID_HVEICULO)
    JOIN HVEICEMP ON (HVEICEMP.IDHVEICEMP = CT.IDHVEICEMP)
	JOIN MODELO ON (HVEICULO.CODEMODE = MODELO.CODEMODE)
	JOIN CONJUNTO ON (HVEICULO.CODECONJ = CONJUNTO.CODECONJ)
WHERE CT.STATUSCT <> 'C' AND YEAR(CT.DATAEMISSAO)=$ano AND MONTH(CT.DATAEMISSAO)=$mes_atual AND HVEICEMP.STAFT='$contrato'
GROUP BY HVEICULO.ID_VEICULO, HVEICULO.PLACA, MODELO.NAME, CONJUNTO.NAME
UNION
SELECT HVEICULO.ID_VEICULO AS ID, HVEICULO.PLACA AS PL, MODELO.NAME AS MODE, CONJUNTO.NAME AS CONJU, SUM(VALFRETE) AS FPESO, SUM(CARRETO.VALFRETE) AS FRETEP
FROM CARRETO
    JOIN HVEICULO ON (HVEICULO.ID_HVEICULO = CARRETO.ID_HVEICULO)
    JOIN HVEICEMP ON (HVEICEMP.IDHVEICEMP = CARRETO.IDHVEICEMP)
	JOIN MODELO ON (HVEICULO.CODEMODE = MODELO.CODEMODE)
...
UNION
SELECT HVEICULO.ID_VEICULO AS ID, HVEICULO.PLACA AS PL, MODELO.NAME AS MODE, CONJUNTO.NAME AS CONJU, SUM(VLR_TOTAL) AS FPESO, SUM(NOTAFAT.VLR_TOTAL) AS FRETEP
FROM NOTAFAT
    JOIN HVEICULO ON (HVEICULO.ID_HVEICULO = NOTAFAT.ID_HVEICULO)
    JOIN HVEICEMP ON (HVEICEMP.IDHVEICEMP = NOTAFAT.ID_HVEICULO)
	JOIN MODELO ON (HVEICULO.CODEMODE = MODELO.CODEMODE)
...
UNION
SELECT HVEICULO.ID_VEICULO AS ID, HVEICULO.PLACA AS PL, MODELO.NAME AS MODE, CONJUNTO.NAME AS CONJU, SUM(NOTASER.VALTOTSERV) AS FPESO, SUM(NOTASER.VALTOTSERV) AS FRETEP
FROM NOTASER
    JOIN HVEICULO ON (HVEICULO.ID_HVEICULO = NOTASER.ID_HVEICULO)
    JOIN HVEICEMP ON (HVEICEMP.IDHVEICEMP = NOTASER.IDHVEICEMP)
    JOIN MODELO ON (HVEICULO.CODEMODE = MODELO.CODEMODE)
...
GROUP BY HVEICULO.ID_VEICULO, HVEICULO.PLACA, MODELO.NAME, CONJUNTO.NAME
 ...
 ORDER BY SUM(FPESO) DESC";
            ...
            while ($dados_placa = db2_fetch_array($db2_placa)){
                $kml = 0;
                $sql_meta = mssql_query("SELECT * FROM meta WHERE meta_descricao='$dados_placa[3]'");
                $dados_meta = mssql_fetch_array($sql_meta);

                $porc_meta = number_format(($dados_placa[4] / $dados_meta[1]) * 100, 0, ',', '.');
                ...
                $linhaTabela = $linhaTabela . "<tr class='gradeA'>
                        ...
                        2, ',', '.') . "</a></td>
                                    <td align='right'><a href='documentos.php?placa=$dados_placa[1]&mes=$mes_atual&ano=$ano'>" . number_format($dados_placa[5],
                        2, ',', '.') . "</a></td>
            ...
            /*
              ...
              <td align='right'>$dados_meta[1]</td>
             */
        ?>
        <div id="wrapper">
                    ...
                    <div class="grid-24">
                        ...
                        <div class="widget widget-table">
                            ...
                            <div class="widget-header">
                                ...
                                <h3 class="icon chart">Faturamento Placa Terceiro</h3>
                            </div>

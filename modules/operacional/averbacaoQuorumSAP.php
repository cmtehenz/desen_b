        ...
        <script type="text/javascript">
            $(document).ready(function(){
                $(".btn-upload").click(function(e){
                    ...
                    $.getJSON('../../library/ajax/wsAverbacaoQuorum.ajax.php', json, function (retorno) {
                        console.log(retorno);
    ...
    <body>
        <?php
            ...
            foreach (listaCteAverbacao($dtIni, $dtFin) as $cte){
                //PArametro a ser passado para envio ao WEBSERVICE
                $numero = $cte['IDFILIAL'] . '-' . $cte['CTE'] . '-' . $cte['ID'];
                $idFilial = $cte['IDFILIAL'];
                $numCte    = $cte['CTE'];
                //verifica se o conhecimento ja est ana base de dados;
                $button = null;
                if(buscaCteLogAverbacao($idFilial, $numCte) == 0){
                    $button = "<button class='btn btn-tertiary btn-upload' cte='$numero'><span class='icon-upload'></span>Upload</button>";                    
                }else{
                    $button = null;
                }
                ...
                $linhaTabela .=
                    "<tr class='gradeA'>
                        ...
                        <td>$status</td>
                        <td>$button</td>
                        <td>".buscaNomeSeguradoraCtrc($cte[IDFILIAL], $cte[CTE])."</td>
                    </tr>";
            }
        ?>
        <div id="wrapper">

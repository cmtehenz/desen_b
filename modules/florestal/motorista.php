...
<html class="no-js" lang="pt-br">
    <head>
        <title>BID - Análise por motorista</title>
    ...
    <body>
        <?php
             ...
             * **************************************** */
            $sql_motorista = mssql_query("SELECT idMotorista, COUNT(*), SUM(peso), SUM((peso/1000)*valor)
                                FROM flr.carregamento
                                WHERE YEAR(data)=$ano AND month(data)=$mes_atual
                                GROUP BY idMotorista ORDER BY SUM(peso) DESC");
            while ($dadosMotorista = mssql_fetch_array($sql_motorista)){

                $sqlNomeMotorista = mssql_query("SELECT top 1 nome FROM flr.motorista WHERE idMotorista = $dadosMotorista[0]");
                $dadosNomeMotorista = mssql_fetch_array($sqlNomeMotorista);

                $peso = number_format($dadosMotorista[1], 0, ',', '.');
                $linhaTabela = $linhaTabela . "<tr class='gradeA'>
                                    <td><a href='sublists/motorista.php?motorista=$dadosMotorista[0]&mes=$mes_atual&ano=$ano'>$dadosNomeMotorista[0]</a></td>
                                    <td align='right'>$dadosMotorista[1]</td>
                                    <td align='right'>" . number_format(($dadosMotorista[2] / 1000), 0, ',', '.') . "</td>
                                    <td align='right'>" . number_format((($dadosMotorista[2] / 1000) / $dadosMotorista[1]), 0, ',', '.') . "</td>
                                    <td align='right'>" . number_format($dadosMotorista[3] / ($dadosMotorista[2] / 1000), 0, ',', '.') . "</td>
                                    <td align='right'>" . number_format($dadosMotorista[3], 2, ',', '.') . "</td>
                                </tr>";

                //TITULO PARA GRAFICO
                $titulo = $titulo . "<th>$dadosNomeMotorista[0]</th>";

                /*                 * **************************** */
                //REALIZADO PARA O GRAFICO
                $graf_realizado = $graf_realizado . '<td>' . $dadosMotorista[2] . '</td>';
            }
            $graf_realizado = $graf_realizado . '<td>0</td>';

            $sqlTotal = mssql_query("SELECT sum(carregamento.peso), COUNT(*), SUM((peso/1000)*valor)
                                FROM [BI].[flr].[carregamento]
                                WHERE YEAR(data)=$ano AND month(data)=$mes_atual
                                ");
            $dados_soma = mssql_fetch_array($sqlTotal);
                    ...
                    <div class="grid-24">
                        ...
                        <form action="#" method="post" name="form1">
                            <div class="field">
                                ...
                                <br><br><b>Periodo Selecionado: <?php echo $dadosMesSelecionado[1] . '/ ' . $ano; ?>.</b>
                        ...
                        <div class="widget widget-table">
                            ...
                            <div class="widget-header">
                                ...
                                <h3 class="icon chart">Carregamento Motorista</h3>
                            </div>
                        ...
                        <div class="widget">
                            ...
                            <div class="widget-header">
                                ...
                                <h3 class="icon chart">PESO CARREGADO</h3>
                            </div>

                            <div class="widget-content">
                                <table class="stats" data-chart-type="bar" data-chart-colors="">
                                    <caption><?php echo $ano; ?> Peso Carregado</caption>
                                    <thead>
                                        <tr>
                                            <td>&nbsp;</td>
                                            <?php echo $titulo; ?>

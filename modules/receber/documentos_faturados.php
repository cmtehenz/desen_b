    ...
    <body>
        <?php
            ...
            while ($dadosMes = mssql_fetch_array($sqlMes)){
                $valorCTe = contasReceberCtFaturado($ano, $dadosMes[0], $status);
                $valorFCR = contasReceberCarretoFaturado($ano, $dadosMes[0], $status);
                $valorNFV = contasReceberNFVendaFaturado($ano, $dadosMes[0], $status);
                $valorNFS = contasReceberNFServFaturado($ano, $dadosMes[0], $status);
                $valorNFD = contasReceberNFDebFaturado($ano, $dadosMes[0], $status);
                ...
                $totalSemana = str_replace(',', '.', $valorCTe) + str_replace(',', '.', $valorFCR) + str_replace(',', '.', $valorNFV) +
                    str_replace(',', '.', $valorNFS) + str_replace(',', '.', $valorNFD);

                $linhaTabela .=
                    "<tr class='gradeA'>
                        <td><a href='sublists/documentos_cliente.php?status=$status&ano=$ano&mes=$dadosMes[0]'>$dadosMes[1]</a></td>
                        ...
                        <td align='right'>" . numberFormatDB2($totalSemana) . "</td>
                    </tr>";
            }

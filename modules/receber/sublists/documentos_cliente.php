...
<html class="no-js" lang="pt-br">
    ...
    <body>
        <?php
            ...
            $tituloPagina = "DOCUMENTOS " . (($status != 'faturado') ? "NÃO" : "") . " FATURADOS POR CLIENTE";
            ...
            // Busca os documentos agrupados por cliente
            $listaClientes = contasReceberDocsFaturado($ano, $mes, $status);

            foreach ($listaClientes as $key => $cliente){
                // Busca Razão Social
                $dadosCliente = getConsultaSQL(sprintf($sqlRazaoSocial, $cliente[1]));
                ...
                $filtroAdicional = " AND C.CNPJ_CPF LIKE '$cliente[1]%' ";

                $valorCTe = contasReceberCtFaturado($ano, $mes, $status, false, $filtroAdicional);
                $valorFCR = contasReceberCarretoFaturado($ano, $mes, $status, false, $filtroAdicional);
                $valorNFV = contasReceberNFVendaFaturado($ano, $mes, $status, false, $filtroAdicional);
                $valorNFS = contasReceberNFServFaturado($ano, $mes, $status, false, $filtroAdicional);
                $valorNFD = contasReceberNFDebFaturado($ano, $mes, $status, false, $filtroAdicional);

                $totalCTe += str_replace(',', '.', $valorCTe);
                $totalFCR += str_replace(',', '.', $valorFCR);
                $totalNFV += str_replace(',', '.', $valorNFV);
                $totalNFS += str_replace(',', '.', $valorNFS);
                $totalNFD += str_replace(',', '.', $valorNFD);

                $linhaTabela .=
                    "<tr class='gradeA'>
                        <td><a href='documentos.php?status=$status&ano=$ano&mes=$mes&cliente=$cliente[1]'>$razaoSocial</td>
                        <td align='right'>" . numberFormatDB2($valorCTe) . "</td>
                        <td align='right'>" . numberFormatDB2($valorFCR) . "</td>
                        <td align='right'>" . numberFormatDB2($valorNFV) . "</td>
                        <td align='right'>" . numberFormatDB2($valorNFS) . "</td>
                        <td align='right'>" . numberFormatDB2($valorNFD) . "</td>
                        <td align='right'>" . numberFormatDB2($cliente[0]) . "</td>
                    </tr>";
            }
                                ...
                                <table class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>CLIENTE</th>
                                            <th width="12%">CT-e</th>
                                            <th width="12%">Carreto</th>
                                            <th width="12%">NF Venda</th>
                                            <th width="12%">NF Serviço</th>
                                            <th width="12%">NF Débito</th>
